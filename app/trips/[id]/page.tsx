...
export default async function TripPage({ params }: { params: { id: string } }) {
  ...
  return (
    <main className="-mt-safe-top pb-10">
      {trip.cover_url ? (
        <>
          <div className="relative h-72 w-full overflow-hidden">
            ...
            <div className="top-safe absolute right-4">
              <ShareTrip code={trip.invite_code} name={trip.name} />
            </div>
            {isOwner && <CoverEdit tripId={trip.id} hasCover />}
          </div>
          <div className="mt-5 px-5">
            <TitleCard trip={trip} members={members ?? []} isOwner={isOwner} lang={lang} />
          </div>
        </>
      ) : (
        <div className="px-5 pt-6">
          <div className="flex items-center justify-between">
            <Link href="/" className="text-sm text-muted">← {tt("trip.back")}</Link>
      ...
      <div className="mt-5 px-5">
        <p className="mb-2 text-xs font-bold uppercase tracking-widest text-muted">{tt("trip.album")}</p>
...
function TitleCard({ trip, members, isOwner, lang }: { trip: any; members: any[]; isOwner: boolean; lang: Lang }) {
  const tt = (key: Parameters<typeof translate>[0]) => translate(key, lang);
  return (
    <div className="rounded-chunk bg-card px-6 pb-6 pt-7 shadow-pop">
      ...
      {trip.start_date && (
        <p className="mt-2 text-sm text-muted">{formatRange(trip.start_date, trip.end_date, lang)}</p>
      )}
      <div className="mt-4 flex items-center justify-between gap-3">
        <MemberAvatars members={members} lang={lang} />
...
function MemberAvatars({ members, lang }: { members: any[]; lang: Lang }) {
  const tt = (key: Parameters<typeof translate>[0]) => translate(key, lang);
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <p className="text-xs text-muted">
        {members.length} {members.length === 1 ? tt("trip.person") : tt("trip.persons")}
      </p>
    </div>
  );
}

function formatRange(start: string, end?: string | null, lang: Lang = "no") {
  const locale = lang === "en" ? "en" : "no";
  const s = new Date(start).toLocaleDateString(locale, { day: "numeric", month: "long" });
  if (!end) return s;
  const e = new Date(end).toLocaleDateString(locale, { day: "numeric", month: "long", year: "numeric" });
  return `${s} – ${e}`;
}
